...
const CBB_LEAGUE = {
  ...
  groups:"50",    // groups=50 returns all D1 games (confirmed from ESPN response)
  limit:300,
  isCBB:true,
};
...
// CBB fetch with automatic fallback to smaller limit if server rejects
async function fetchCBBSafe(){
  // Try progressively smaller limits until it fits under the 1.5MB cap
  const limits = [200, 100, 60];
  for(const limit of limits){
    try{
      const data = await espnFetch(
        CBB_LEAGUE.sport, CBB_LEAGUE.league, "scoreboard",
        {groups: CBB_LEAGUE.groups, limit}
      );
      const events = (data && data.events) || [];
      console.log("[CBB] fetched "+events.length+" games with limit="+limit);
      return events;
    }catch(err){
      if(err.message === "PAYLOAD_TOO_LARGE"){
        console.warn("[CBB] Too large at limit="+limit+", retrying smaller…");
        continue;
      }
      throw err; // non-size error, bail
    }
  }
  console.warn("[CBB] All limits failed, returning empty");
  return [];
}
...
function parseGame(ev,sport,league){
  try{
    ...
    // Conference — ESPN puts it on comp.groups for CBB scoreboard
...
function SportsBoard(){
  ...
  const fetchCBB=useCallback(async()=>{
    const events = await fetchCBBSafe();

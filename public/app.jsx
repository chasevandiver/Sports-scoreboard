...
// ── ALL LEAGUES ───────────────────────────────────────────────────────
// CBB uses ESPN's full scoreboard which includes all D1 conferences
// when we DON'T pass a groups= filter. ESPN defaults to top-25 only
// when called without ?groups=50. groups=50 = all D1.
const ALL_LEAGUES = [
  { sport:"basketball", league:"nba",                     label:"NBA",   icon:"🏀", accent:"#C9082A", groups:null },
  { sport:"basketball", league:"mens-college-basketball", label:"NCAAM", icon:"🏀", accent:"#1A4A8A", groups:"50" }, // 50 = all D1
  ...
  { sport:"football",   league:"college-football",        label:"NCAAF", icon:"🏈", accent:"#8B2500", groups:"80" }, // all FBS
...
function GameCard({game,flash,rowH}) {
  ...
  const scoreSz    = Math.floor(rowH*0.28);
  const abbrSz     = Math.floor(rowH*0.15);
  const leaderSz   = Math.floor(rowH*0.085);
  const leaderPtSz = Math.floor(rowH*0.11);
  const logoSz     = Math.floor(rowH*0.24);
  const cardW      = Math.floor(rowH*2.0);
...
// ── SETTINGS PANEL ────────────────────────────────────────────────────
function SettingsPanel({allLeagues,enabledSet,hasGames,onToggle,onClose}) {
  return (
    <div style={{
      ...
      <div style={{
        ...
        <p style={{fontSize:13,color:"rgba(255,255,255,0.35)",marginBottom:18,lineHeight:1.5}}>
          Toggle sports on/off. Only sports with games today appear.
          Disabled sports are skipped in the rotation.
        </p>

        {/* League toggles */}
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {allLeagues.map(lg=>{
            const enabled  = enabledSet.has(lg.league);
            const hasGame  = hasGames(lg.league);
            return (
              <div key={lg.league}
                onClick={()=>onToggle(lg.league)}
                style={{
                  display:"flex",alignItems:"center",gap:14,
                  padding:"12px 16px",borderRadius:10,cursor:"pointer",
                  background: enabled ? `${lg.accent}22` : "rgba(255,255,255,0.04)",
                  border:`1.5px solid ${enabled ? lg.accent : "rgba(255,255,255,0.08)"}`,
                  transition:"all 0.2s",
                  opacity: hasGame ? 1 : 0.45,
                }}>
                <span style={{fontSize:24}}>{lg.icon}</span>
                <div style={{flex:1}}>
                  ...
                  <div style={{fontSize:12,color:"rgba(255,255,255,0.35)",marginTop:1}}>
                    {hasGame ? `Games today` : "No games today"}
                  </div>
                </div>
                {/* Toggle pill */}
                <div style={{
                  width:44,height:24,borderRadius:12,position:"relative",
                  background: enabled ? lg.accent : "rgba(255,255,255,0.15)",
...
function SportLabel({meta,rowH}) {
  return (
    <div style={{
      ...
      <span style={{fontSize:Math.floor(rowH*0.24),lineHeight:1}}>{meta.icon}</span>
      <span style={{fontSize:Math.floor(rowH*0.095),fontWeight:900,color:"#fff",
...
function SportsBoard() {
  ...
  // enabledLeagues: set of league strings the user has turned ON
  // Default: all leagues enabled
  const [enabledLeagues, setEnabledLeagues] = useState(
    ()=>new Set(ALL_LEAGUES.map(l=>l.league))
  );
  ...
  const fetchLeague = useCallback(async(lg)=>{
    const params = lg.groups ? { groups:lg.groups, limit:300 } : { limit:200 };
    ...
    const updatedGames = games.map(g=>{
      ...
      if (alert) {
        ...
        alertTimers.current[g.id] = setTimeout(()=>{
          setAllGames(prev=>({
            ...prev,
            [lg.league]:(prev[lg.league]??[]).map(x=>x.id===g.id?{...x,alert:null}:x),
          }));
        }, ALERT_DURATION_MS);
      }
    ...
    return { league:lg.league, games:updatedGames };
  },[]);
  ...
  const loadAllScores=useCallback(async()=>{
    ...
    results.forEach((r,i)=>{
      if(r.status==="fulfilled"){
        ...
        if(games.length>0 && enabledLeagues.has(league)) withGames.push(ALL_LEAGUES[i]);
      }
    });
    ...
    <>
      ...
      {showSettings&&(
        <SettingsPanel
          ...
          hasGames={l=>(allGames[l]?.length??0)>0}
